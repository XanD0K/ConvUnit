...
__all__ = ["_manage_aliases"]


def _manage_aliases(data: DataStore, aliases_data: Optional[AliasesData] = None) -> str:
    """Handles adding and removing aliases for unit types."""

    # Interactive approach
    if aliases_data is None:
        aliases_data = AliasesData(unit_group = _get_unit_group(data))

        aliases_data.unit_type = _get_user_input(
        ...
        if not existing_aliases:
            ...
            aliases_data.action = _get_user_input(
                f"Existing aliases for '{aliases_data.unit_type}': {existing_aliases}. "
        ...
        aliases_data.alias = _get_user_input(prompt)
