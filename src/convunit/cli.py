...
def _handle_cli(data: DataStore, args: list[str]) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="ConvUnit", description="Convert multiple types of units"
    )
    ...
    history_parser.add_argument("--reset", "-r", action="store_true", help="Resets history log")
    # 'types' command
    ...
    # 'convert' command 
    convert_parser = subparser.add_parser("convert", aliases=["c"], help="Converts values")
    ...
    # 'base' command
    base_parser = subparser.add_parser("base", aliases=["b"], help="Displays base unit")
    ...
    # 'reset' command
    reset_parser = subparser.add_parser("reset", help="Reset data files")
    ...
    match parsed_args.command:
        ...
        case "convert" | "c":
            ...
            if conversion_data.unit_group == "time":
                ...
                if len(parsed_args.args) not in [2, 3]:
                    raise InvalidInputError("Invalid format! Use: <from_type> <to_type> [amount]")
        ...
        case "manage-group" | "mg":
            manage_group_data: ManageGroupData = ManageGroupData(
                ...
                new_base_unit = (
                    parsed_args.new_base_unit.lower()
...
def cli_entrypoint() -> None:
    """
    Entry point for the `convunit` command when the package is installed via pip.

    This allows running `convunit` directly from the terminal
    instead of using `python -m src.convunit`.
    """
