...
from .utils import _get_user_input, _get_unit_group


__all__ = ["_manage_group"]


def _manage_group(data: DataStore, manage_group_data: Optional[ManageGroupData] = None) -> str:
    """Handles creation and removal of custom unit groups."""

    # Interactive approach
    if manage_group_data is None:
        manage_group_data = ManageGroupData(unit_group=None, action=None, new_base_unit=None)

        manage_group_data.action = _get_user_input(
            f"Existing groups: {', '.join(data.units.keys())}. "
            f"What do you want to do? ('add' or 'remove') "
        )
        manage_group_data._validate_action()

        if manage_group_data.action == "add":
            manage_group_data.unit_group = _get_user_input(f"New unit group: ")
            manage_group_data._validate_add_action(data)

            manage_group_data.new_base_unit = _get_user_input(
